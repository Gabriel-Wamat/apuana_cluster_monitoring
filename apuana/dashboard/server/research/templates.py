...
from ..config import RESEARCH_TEMPLATES_ROOT
...
def list_research_templates() -> list[dict]:
    if not RESEARCH_TEMPLATES_ROOT.exists():
        return []
    templates = []
    for path in sorted(RESEARCH_TEMPLATES_ROOT.glob("*/template.json")):
        try:
...
def get_research_template(template_id: str) -> dict:
    ...
    path = (RESEARCH_TEMPLATES_ROOT / wanted / "template.json").resolve()
    try:
        path.relative_to(RESEARCH_TEMPLATES_ROOT.resolve())
