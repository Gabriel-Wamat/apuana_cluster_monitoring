...
APUANA_DIR="$(cd "${BIN_DIR}/.." && pwd)"
STREAMLIT_DIR="${APUANA_DIR}/streamlit_dashboard"
SCRIPT_DIR="$APUANA_DIR"
# shellcheck disable=SC1091
source "${APUANA_DIR}/lib/lib_env.sh"

REQ="${STREAMLIT_DIR}/requirements.txt"
DASH="${STREAMLIT_DIR}/app.py"
PAINEL="${BIN_DIR}/painel_slurm.sh"
...
start_streamlit_bg() {
  local port="$1"
  local logf="${APUANA_DIR}/streamlit-slurm-monitor.log"
  local pidf="${APUANA_DIR}/streamlit-slurm-monitor.pid"
  local py="${MONITOR_VENV}/bin/python"
